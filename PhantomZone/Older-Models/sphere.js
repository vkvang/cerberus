	...
	function advance(pt, drawingState) {
		...
		switch (pt.state) {
		    case 0: // Waiting at Point
		        ...
		        break;
		    case 1: // Taking off
		        if (pt.position < height) {
		            var up = delta * verticalSpeed;
		            pt.position[1] = Math.min(height, pt.position[1] + up);
		        } else {
		    ...
		    case 3: // Reach goal
		        ...
		        if (pt.position > dest) {
		            var down = delta;
		            pt.position[1] = Math.max(dest, pt.position[1] - down);
		        } else {
		            pt.state = 0;
...
// Render the objects
grobjects.push(new Sphere("Homosapien",[],0.75, [4.65,0.0,0.0]));

grobjects.push(new Point([3,0.2,3]));
grobjects.push(new Point([3,0.2,-3]));
grobjects.push(new Point([-3,0.2,3]));
grobjects.push(new Point([-3,0.2,-3]));
